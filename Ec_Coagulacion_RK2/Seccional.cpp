...
long double Seccional::densidad_media0(int i, long double t) {
    ...
    for (int i = 1; i <= k; i++) {
        sum1 = sum1 + densidadN0(xRange[3 * i - 2], t) + densidadN0(xRange[3 * i - 1], t);
        if (i < k) {
            sum2 = sum2 + densidadN0(xRange[3 * i], t);
        }
    }
    suma = (3 * precision / 8)*(densidadN0(xRange[0], t) + 3 * sum1 + 2 * sum2 + densidadN0(xRange[nParticiones], t));
...
long double Seccional::coefB1(int i, int j, int l) {
    ...
    auto integrando = [&](long double x, long double y) {
        return (thetaInfSup(vInf, vSup, fInverse_point(y) + fInverse_point(x)) *
                pow(fInverse_point(y) + fInverse_point(x), gamma) * beta(fInverse_point(y), fInverse_point(x)))
                / (alfa * pow(fInverse_point(y), gamma) * pow(fInverse_point(x), gamma));
    };
     * */
    auto integrando = [&](long double x, long double y) {
        return thetaInfSup(vInf, vSup, fInverse_point(y) + fInverse_point(x))*(fInverse_point(y) + fInverse_point(x));
    ...
    long double ya;
    long double yb;
    for (int x = 0; x < nParticionesX - 1; x++) {
        xa = xRange[x];
        xb = xRange[x + 1];
        xm = (xa + xb) / 2;
        for (int y = 0; y < nParticionesY - 1; y++) {
            ya = yRange[y];
            yb = yRange[y + 1];
            //ym = (ya + yb) / 2;
            /*
            suma = suma + (16 * integrando(xm, ym) + 4 * integrando(xm, yb)
                    + 4 * integrando(xm, ya) + 4 * integrando(xb, ym)
                    + integrando(xb, yb) + integrando(xb, ya)
                    + 4 * integrando(xa, ym) + integrando(xa, yb)
                    + integrando(xa, ya))*((xb - xa)*(yb - ya) / 36);
             */

            //suma=suma + 0.25*(integrando(xa,ya)+integrando(xa,yb)+integrando(xb,ya)+integrando(xb,yb));

            suma = suma + (16 * thetaInfSup(vInf, vSup, fInverse_point(ym) + fInverse_point(xm)) +
                    4 * thetaInfSup(vInf, vSup, fInverse_point(xm) + fInverse_point(yb))
                    + 4 * thetaInfSup(vInf, vSup, fInverse_point(ya) + fInverse_point(xm))
                    + 4 * thetaInfSup(vInf, vSup, fInverse_point(ym) + fInverse_point(xb))
                    + thetaInfSup(vInf, vSup, fInverse_point(yb) + fInverse_point(xb))
                    + thetaInfSup(vInf, vSup, fInverse_point(ya) + fInverse_point(xb))
                    + 4 * thetaInfSup(vInf, vSup, fInverse_point(ym) + fInverse_point(xa))
                    + thetaInfSup(vInf, vSup, fInverse_point(yb) + fInverse_point(xa))
                    + thetaInfSup(vInf, vSup, fInverse_point(ya) + fInverse_point(xa)))*((xb - xa)*(yb - ya) / 36);


            // suma = suma + (xb - xa)*(yb - ya)*0.25 * (integrando(xa, ya) + integrando(xa, yb) + integrando(xb, ya) + integrando(xb, yb));
        }
    }
...
long double Seccional::coefB2(int i, int l) {
    ...
    /*
        auto integrando = [&](long double x, long double y) {
            return ((thetaInf(vInf, fInverse_point(y) + fInverse_point(x)) *
                    pow(fInverse_point(y), gamma) - thetaSup(vSup, fInverse_point(y) + fInverse_point(x))
     *(pow(fInverse_point(y) + fInverse_point(x), gamma) - pow(fInverse_point(y), gamma))) *
                    beta(fInverse_point(y), fInverse_point(x))) / (alfa * pow(fInverse_point(y), gamma) * pow(fInverse_point(x), gamma));
        };
     * */
    auto integrando = [&](long double x, long double y) {
        return thetaInf(vInf, fInverse_point(y) + fInverse_point(x))*(fInverse_point(y) + fInverse_point(x));
    };
...
long double Seccional::coefB3(int l) {
    ...
    /*
    auto integrando = [&](long double x, long double y) {
        return ((thetaInf(vInf, fInverse_point(y) + fInverse_point(x))*(pow(fInverse_point(y), gamma)
                + pow(fInverse_point(x), gamma)) + thetaSup(vSup, fInverse_point(y) + fInverse_point(x))*((pow(fInverse_point(y), gamma)
                + pow(fInverse_point(x), gamma)) - pow(fInverse_point(y) + fInverse_point(x), gamma))) *
                beta(fInverse_point(y), fInverse_point(x))) / (alfa * pow(fInverse_point(y) * fInverse_point(x), gamma));
    };
     * */
    auto integrando = [&](long double x, long double y) {
        return (thetaInf(vInf, fInverse_point(y) + fInverse_point(x))*2 + thetaSup(vSup, fInverse_point(y) + fInverse_point(x)))
                *(fInverse_point(y) + fInverse_point(x));
...
long double Seccional::coefB4(int i, int l) {
    ...
    /*
        auto integrando = [&](long double x, long double y) {
            return (pow(fInverse_point(y), gamma) * beta(fInverse_point(y), fInverse_point(x)))
                    / (alfa * pow(fInverse_point(y) * fInverse_point(x), gamma));
        };
     * */

    auto integrando = [&](long double x, long double y) {
        return (fInverse_point(y) + fInverse_point(x));
    ...
    while (domTiempo[z - 1] < tFinal) {
        ...
        if (tiempoAcumulado >= 1799) {
            ...
            for (int j = 0; j < m; j++) {
                ...
                error = error + (v[j + 1] - v[j]) * abs(valorEscribir - densidadN0(vi[j], domTiempo[z]));
            }
