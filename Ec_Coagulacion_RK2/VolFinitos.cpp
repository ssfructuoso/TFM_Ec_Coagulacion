...
long double VolFinitos::fIni(long double x, long double t) {
    ...
    //long double M0 = (2.0 * M00) / (2 + M00 * t);
    //n = M0 * M0 * exp(-M0 * x);
...
long double VolFinitos::f_media0(int i, long double t) {
    ...
    int k = nParticiones / 3;
    for (int i = 1; i <= k; i++) {
        sum1 = sum1 + fIni(xRange[3 * i - 2], t) + fIni(xRange[3 * i - 1], t);
        if (i < k) {
            sum2 = sum2 + fIni(xRange[3 * i], t);
        }
    }
    suma = (3 * precision / 8)*(fIni(xRange[0], t) + 3 * sum1 + 2 * sum2 + fIni(xRange[nParticiones], t));
...
long double VolFinitos::g_media0(int i, long double t) {
    ...
    for (int i = 1; i <= k; i++) {
        sum1 = sum1 + gIni(xRange[3 * i - 2], t) + gIni(xRange[3 * i - 1], t);
        if (i < k) {
            sum2 = sum2 + gIni(xRange[3 * i], t);
        }
    }
    suma = (3 * precision / 8)*(gIni(xRange[0], t) + 3 * sum1 + 2 * sum2 + gIni(xRange[nParticiones], t));
    suma = suma / (x12[i + 1] - x12[i]);
    return suma;
}
...
long double VolFinitos::A(int j, long double x_k) {
    ...
    for(int i=0; i<nParticiones;i++){
        suma=suma+ 0.5*(ker(xRange[i], x_k)/xRange[i] + ker(xRange[i+1], x_k)/xRange[i+1])*(xRange[i+1]-xRange[i]);
    }
     * */
    ...
    int k = nParticiones / 3;
    for (int i = 1; i <= k; i++) {
        sum1 = sum1 + ker(xRange[3 * i - 2], x_k) / xRange[3 * i - 2] + ker(xRange[3 * i - 1], x_k) / xRange[3 * i - 1];
        if (i < k) {
            sum2 = sum2 + ker(xRange[3 * i], x_k) / xRange[3 * i];
        }
    }
    suma = (3 * precision / 8)*(ker(xRange[0], x_k) / xRange[0] + 3 * sum1 + 2 * sum2 + ker(xRange[ nParticiones], x_k) / xRange[ nParticiones]);
