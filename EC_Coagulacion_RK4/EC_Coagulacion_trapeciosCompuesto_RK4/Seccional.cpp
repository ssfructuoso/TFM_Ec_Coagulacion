...
long double Seccional::coefB1(int i, int j, int l) {
    ...
    long double sum_aux;
    //###### Bloque 1
    suma=integrando(xRange[0],yRange[0]);
    sum_aux=0;
    for(int i=1;i<=nParticionesX-1;i++){
        sum_aux+=integrando(xRange[i],yRange[0]);
    }
    suma+=2*sum_aux;
    suma+=integrando(xRange[nParticionesX],yRange[0]);
    
    //#####Bloque2
    sum_aux=0;
    for(int j=1;j<=nParticionesY-1;j++){
        sum_aux+=integrando(xRange[0],yRange[j]);
    } 
    
    for(int j=1;j<=nParticionesY-1;j++){
        for(int i=1;i<=nParticionesX-1;i++){
            sum_aux+=2*integrando(xRange[i],yRange[j]);
        }
    }
    
    for(int j=1;j<=nParticionesY-1;j++){
        sum_aux+=integrando(xRange[nParticionesX],yRange[j]);
    } 
    suma+=2*sum_aux;
    
    ////Bloque 3
    suma+=integrando(xRange[0],yRange[nParticionesY]);
    sum_aux=0;
    for(int i=1;i<=nParticionesX-1;i++){
        sum_aux+=2*integrando(xRange[i],yRange[nParticionesY]);
    } 
    suma+=sum_aux;
    suma+=integrando(xRange[nParticionesX],yRange[nParticionesY]);
    
    suma*=(precisionX*precisionY)/4;
