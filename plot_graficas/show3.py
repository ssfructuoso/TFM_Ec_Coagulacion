...
datosTxt='FEBRERO/seccional/RK3/trapecios_compuesto/FINAL/2s-seccional_RK3_SUMA_trapecios_compuesto_200.txt'
...
plt.ylim(-0.01,5)
plt.legend()
plt.title('Ker(x,y)=x+y (Seccional, Nx=200)')
plt.xlabel('x')
plt.ylabel('x$\/$n(x,t)')
...
plt.plot(np.log(xi),np.log(g[pos4,:]),'m',label="t="+str(tiempo4))

plt.legend()
plt.title('Ker(x,y)=x+y (Seccional, Nx=200)')
plt.xlabel('log(x)')
plt.ylabel('log(x$\/$n(x,t))')
